...
/* Returns the size of DIR (not its content) in bytes. */
off_t dir_length(struct dir* dir) {
  ASSERT(dir != NULL);
  return inode_length(dir->inode);
}

/* Returns the number of entries in DIR (excluding built-ins). */
off_t dir_entry_count(struct dir* dir) {
  struct dir_entry e;
  size_t count = 0;
